...
export const cutLastLine = (text: string, width: number, start: number, caculateWidth: WidthCaculator, wordBased?: boolean, ellipsis?: boolean): string => {
  ...
  const idx = wordBased ? nextWordsEnding(text, width, start, caculateWidth) : nextEnding(text, width, start, caculateWidth);
...
export const seperateText2MultiLines = (text: string, width: number, caculateWidth: WidthCaculator, targetLines: number, wordBased?: boolean, ellipsis?: boolean): string[] => {
  ...
  for (let i = 0; i < targetLines - 1; i++) {
    ...
    const idx = wordBased ? nextWordsEnding(text, width, l, caculateWidth) : nextEnding(text, width, l, caculateWidth);
  ...
  const lastLine = cutLastLine(text, width, l, caculateWidth, wordBased, ellipsis);
